...
from picutils import PICTimer, PICTimerWith
from typing import Optional

doTopKOp: bool = False

from pictest.raycnt import RayCounter
rayCounter = RayCounter(grid=grid)

while True:
    with PICTimer.getTimer("svox") as picTimer:

        torch.cuda.synchronize()
        picTimer.showTime("shuffle_rays_start")

        dset.shuffle_rays()

        torch.cuda.synchronize()
        picTimer.showTime("shuffle_rays_end")
        ...
        if epoch_id % max(factor, args.eval_every) == 0: #and (epoch_id > 0 or not args.tune_mode):
            ...
            torch.cuda.synchronize()
            picTimer.showTime("eval_step_start")
            eval_step()
            gc.collect()
            torch.cuda.synchronize()
            picTimer.showTime("eval_step_end")

        def calculateInitLoss():
            if not hasattr(dset, 'ray_loss'):
                return
            print('init loss')
            with tqdm(total=epoch_size) as pbar:
                for iter_id, batch_begin in enumerate(range(0, epoch_size, args.batch_size)):
                    ...
                    rgb_pred = grid.volume_render(rays)
                    
                    loss_mse = ((rgb_gt - rgb_pred) ** 2).mean(dim=1)

                    dset.ray_loss[batch_begin: batch_end] = loss_mse
        ...
        def train_step(timer: PICTimerWith=None):
            global doTopKOp
            print('Train step')
            ...
            if hasattr(dset, 'ray_loss'):
                dset.shuffle_rays(sort=False)

            with open('outputs/no_ucb.txt', 'w') as fFile:
            # if True:
                ...
                with tqdm(total=epoch_size) as pbar:
                    for iter_id, batch_begin in enumerate(range(0, epoch_size, args.batch_size)):
                        ...
                        # idx1 = None
                        if hasattr(dset, 'ray_loss') and doTopKOp:
                            # batch_end = min(batch_begin + args.batch_size, epoch_size)
                            calculateInitLoss()
                            # grid.density_data.data = torch.nn.functional.relu(grid.density_data.data - grid.opt.sigma_thresh) + grid.opt.sigma_thresh
                            # negativeDensityIndex = grid.density_data.data < grid.opt.sigma_thresh
                            # grid.density_data.data[negativeDensityIndex] = 0
                            sample_score = dset.ray_loss# + args.ucb_factor * (batch_end / (dset.ray_sample_cnt + 1e-10))**0.5
                            val, idx1 = torch.topk(sample_score, args.batch_size)
                            batch_origins = dset.rays.origins[idx1]
                            batch_dirs = dset.rays.dirs[idx1]
                            rgb_gt = dset.rays.gt[idx1]
                        ...
                        rgb_pred = grid.volume_render(rays)
                        loss_mse = ((rgb_gt - rgb_pred) ** 2).mean(dim=1)
                        val, idx2 = torch.topk(loss_mse, args.batch_size)
                        batch_origins = batch_origins[idx2]
                        batch_dirs = batch_dirs[idx2]
                        rgb_gt = rgb_gt[idx2]
                        rays = svox2.Rays(batch_origins, batch_dirs)

                        def outputRayCountAndLossMap(path, batchSize=None):
                            for img_id in tqdm(range(dset.n_images)):
                                #build info
                                ...
                                lossDiff = ((im - dset.gt[img_id].to(device))**2).sum(dim=2)

                                # visit counts
                                rays = cam.gen_rays()
                                all_visit_out = []
                                for batch_start in range(0, cam.height * cam.width, batchSize):
                                    visit_couts_part = rayCounter.count(rays[batch_start:batch_start+batchSize], addToTotal=False)
                                    # all_visit_out.append(visit_couts_part['raysum'])
                                    all_visit_out.append(visit_couts_part['raymean'])

                                all_visit_out = torch.cat(all_visit_out, dim=0).reshape(cam.height, cam.width)

                                # normalize
                                lossDiff = (myNormalize(-lossDiff) * 255).detach().cpu().numpy().astype(np.uint8)
                                lossDiff = cv2.applyColorMap(lossDiff, cv2.COLORMAP_RAINBOW)
                                all_visit_out = (myNormalize(-all_visit_out) * 255).cpu().numpy().astype(np.uint8)
                                all_visit_out = cv2.applyColorMap(all_visit_out, cv2.COLORMAP_RAINBOW)

                                # merge and output
                                outputImage = np.zeros((cam.height*2, cam.width*2, 3), dtype=np.uint8)
                                outputImage[:cam.height, :cam.width] = cv2.cvtColor((dset.gt[img_id] * 255).cpu().detach().numpy(), cv2.COLOR_BGR2RGB)
                                outputImage[cam.height:, :cam.width] = cv2.cvtColor((im * 255).cpu().detach().numpy(), cv2.COLOR_BGR2RGB)
                                outputImage[:cam.height, cam.width:] = lossDiff
                                outputImage[cam.height:, cam.width:] = all_visit_out

                                # save image
                                cv2.imwrite(os.path.join(path, "%05d.jpg" % (img_id, )), outputImage)

                        # with Timing("volrend_fused"):
                        if True:
                            # renderStartEvent = torch.cuda.Event(enable_timing=True)
                            ...
                            if doTopKOp:
                            # if True:
                                # rayCounter.countAndSave('outputs/raycnt/%08d.npy' % (iter_id, ), rays, addToTotal=True, returnRayInfo=True)

                                # outputRayCountAndLossMap(os.path.join('outputs', 'raycnt', '256'), batchSize=200000)
                                # print('count Done!')

                                def calculateAllRaySampleCount():
                                    result = []
                                    bigBatchSize = 200000
                                    with tqdm(total=epoch_size) as pbar:
                                        for iter_id, batch_begin in enumerate(range(0, epoch_size, bigBatchSize)):
                                            batch_end = min(batch_begin + bigBatchSize, epoch_size)
                                            batch_origins = dset.rays.origins[batch_begin: batch_end]
                                            batch_dirs = dset.rays.dirs[batch_begin: batch_end]
                                            rays = svox2.Rays(batch_origins, batch_dirs)
                                            _, rayCountResult = rayCounter.count(rays, addToTotal=False, countGrid=rayCounter.counts)
                                            result.append(rayCountResult)

                                            pbar.update(bigBatchSize)

                                    return torch.cat(result, dim=0)
                                
                                # tmp = calculateAllRaySampleCount()
                                # tmpVal, tmpIdx = torch.topk(-tmp, args.batch_size)

                                # s1 = set([int(_) for _ in idx1])
                                # s2 = set([int(_) for _ in tmpIdx])

                                # print('overlay', len(s1 & s2) / len(s1))
                                # print(1)


                            # renderEndEvent.record()
                            # torch.cuda.synchronize()

                            # fFile.write('iter: %d\t\t%f ms\n' % (iter_id, renderStartEvent.elapsed_time(renderEndEvent), ))
                        ...
                        if (iter_id + 1) % args.print_every == 0:
                            ...
                            def outputImgs(path, batch=0, psnr=None):
                                c2ws = dset_test.c2w.to(device=device)
                                batch = str(batch)
                                if psnr is not None:
                                    batch = batch + '-' + '%.5f' % (psnr,)
                                basePath = os.path.join('outputs', path, batch)
                                if not os.path.exists(basePath):
                                    os.makedirs(basePath)
                                for img_id in tqdm(range(dset_test.n_images)):
                                    ...
                                    cv2.imwrite(os.path.join(basePath, '{:03d}.png'.format(img_id)), cv2.cvtColor((im * 255).cpu().detach().numpy(), cv2.COLOR_BGR2RGB))

                            def outputImgTopk(path, batch=0):
                                batch = str(batch)
                                basePath = os.path.join('outputs', path, batch)
                                c2ws = dset.c2w.to(device=device)
                                if not os.path.exists(basePath):
                                ...
                                for img_id in tqdm(range(dset.n_images)):
                                    ...
                                    im = im.reshape(w*h, 3)
                                    gt = dset.gt[img_id].reshape(w*h, 3).to(device)
                                    mseScore = ((im - gt)**2).sum(dim=1)
                                    _, topIdx = torch.topk(mseScore, dim=0, k=args.batch_size)
                                    # topY = topIdx.div(w, rounding_mode='floor')
                                    # topX = topIdx % w

                                    gt[topIdx] = torch.tensor([[0, 1., 0]], device=device)

                                    gt = gt.reshape(h, w, 3)
                                    im = im.reshape(h, w, 3)

                                    gt = torch.cat([gt, im], dim=1)

                                    # print(1)

                                    cv2.imwrite(os.path.join(basePath, '{:03d}.png'.format(img_id)), cv2.cvtColor((gt * 255).cpu().detach().numpy(), cv2.COLOR_BGR2RGB))
                                print('Done')

                            path = os.path.join('outputs', 'raycnt', '128_mean', '%08d' % (iter_id + 1, ))
                            os.mkdir(path)
                            outputRayCountAndLossMap(path, batchSize=200000)
                            print('count Done!')

                            # val
                            if args.output_all_after_iter is not None and iter_id >= args.output_all_after_iter:
                            # if False:
                            # if doTopKOp:
                                print('val')
                                ...
                                for img_id in tqdm(range(dset_test.n_images)):
                                    ...
                                    im_gt = dset_test.gt[img_id].to(device=device)
                                    mse = (im - im_gt) ** 2
                                    mse_num : float = mse.mean().item()
                                    psnr = -10.0 * math.log10(mse_num)
                                    avg_psnr += psnr
                                
                                avg_psnr /= dset_test.n_images

                                # fFile.write('epoch %04d\titer %04d\tpsnr %.4f\n' % (epoch_id, iter_id, psnr))
                                # fFile.write('%.4f\n' % (avg_psnr))
                                # print('val', iter_id + 1, 'mean psnr', avg_psnr)

                                # outputImgs('alphaTest64_scale0_25/fixAirAlphaAfterPSNR22', iter_id, psnr=avg_psnr)

                            # if iter_id == 99:
                            #     outputImgs('val_100')
                            #     print('done')
                            # if iter_id >= 200:
                            #     outputImgTopk('topk_low', batch_begin)
                            #     print('done')                                
                        ...
                        # if hasattr(dset, 'ray_loss') and doTopKOp:
                        #     grid.density_data.grad[negativeDensityIndex] = 0
                        ...
                        if hasattr(dset, 'ray_loss') and doTopKOp:
                            if doTopKOp:
                                rgb_pred = grid.volume_render(rays)
                                loss_mse = ((rgb_gt - rgb_pred) ** 2).mean(dim=1)
                                dset.ray_loss[idx1] = loss_mse
                                dset.ray_sample_cnt[idx1] += 1
                            else:
                                batch_end = min(batch_begin + args.batch_size, epoch_size)
                                rgb_pred = grid.volume_render(rays)
                                loss_mse = ((rgb_gt - rgb_pred) ** 2).mean(dim=1)
                                dset.ray_loss[batch_begin: batch_end] = loss_mse
                                dset.ray_sample_cnt[batch_begin: batch_end] += 1
                        if args.start_topk_over_psnr is not None and avg_psnr > args.start_topk_over_psnr:
                        # if False:
                            doTopKOp = True
                            # calculateInitLoss()
                            # print(22)
        ...
        torch.cuda.synchronize()
        picTimer.showTime("train_step_start")
        train_step(picTimer.getTimer("train", autoStart=True))
        gc.collect()
        torch.cuda.synchronize()
        picTimer.showTime("train_step_end")
